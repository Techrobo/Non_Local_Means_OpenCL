...
int main(int argc, char** argv) {
	...
	cl::Program program = OpenCL::loadProgramSource(context, "/zhome/guptasm/Opencl-Basics-Linux/src/OpenCLExercise2_Mandelbrot.cl");
	...
	queue.enqueueNDRangeKernel(mandelbrotKernel, cl::NullRange, cl::NDRange(countX, countY), cl::NDRange(wgSizeX, wgSizeY), NULL, &execution);

	// Copy output data back to host
	cl::Event copy;
	queue.enqueueReadBuffer(d_output, true, 0, size, h_outputGpu.data(), NULL, &copy);

	// Print performance data
	Core::TimeSpan cpuTime = cpuEnd - cpuStart;
	Core::TimeSpan gpuTime = OpenCL::getElapsedTime(execution);
	Core::TimeSpan copyTime = OpenCL::getElapsedTime(copy);
